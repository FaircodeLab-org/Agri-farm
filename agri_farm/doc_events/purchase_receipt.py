        ...
        for item in self.items:
            ...
            for crop in doc.crop:
                if crop.crop_variant == item.item_code:
                    ...
                    if purchased_qty > crop.available_qty_kg:
                        frappe.throw("Purchased quantity exceeds the estimated yield count for crop {0}".format(crop.crop_variant))
                    else:
                        crop.ordered_qty_kg += purchased_qty
                        crop.available_qty_kg = crop.available_qty_kg - purchased_qty
            ...
            for k in doc.animal_husbandry:
                if k.animal_variant == item.item_code:
                    ...
                    yielding_count = k.yielding_count
                                
                    if purchased_qty > k.available_qty_kg:
                        frappe.throw("Purchased quantity exceeds the yielding count for animal {0}".format(k.animal_variant))
                                    
                    else:
                        k.ordered_qty_kg += purchased_qty
                        k.available_qty_kg = k.available_qty_kg - purchased_qty
        doc.save()
        doc.submit()
...
@frappe.whitelist()
def cancel_update_qty(self, method):
    print("submit ok")
