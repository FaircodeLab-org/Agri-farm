...
frappe.pages["plantrich-leaderboar"].on_page_load = (wrapper) => {
	frappe.plantrichleaderboard = new PlantrichLeaderboard(wrapper);

	$(wrapper).bind("show", () => {
		// Get which leaderboard to show
		let doctype = frappe.get_route()[1];
		frappe.plantrichleaderboard.show_leaderboard(doctype);
	});

};
frappe.pages["plantrich-leaderboar"].refresh = (wrapper) => {
...
class PlantrichLeaderboard {
	constructor(parent) {
		frappe.ui.make_app_page({
			parent: parent,
			title: __("Plantrich Leaderboard"),
			single_column: false,
			card_layout: true,
		});
	...
	get_leaderboard_config() {
		...
		frappe
			.xcall("agri_farm.agri_farm.page.plantrich_leaderboar.plantrich_leaderboard.get_leaderboard_config")
			.then((config) => {
	...
	setup_leaderboard_fields() {
		...
		var company = ""
		frappe.call({
			method: "agri_farm.agri_farm.page.plantrich_leaderboar.plantrich_leaderboard.get_company",
			async: false,
			args: {
...
this.company_select = this.page.add_field({
	...
	render_selected_doctype() {
		this.$sidebar_list.on("click", "li", (e) => {
			...
			frappe.set_route("plantrich-leaderboar", this.options.selected_doctype);
			this.make_request();
		});
	}
	...
	get_leaderboard(notify) {
		...
		frappe
			.call("agri_farm.agri_farm.page.plantrich_leaderboar.plantrich_leaderboard.get_energy_point_leaderboard", {
