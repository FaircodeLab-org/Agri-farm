frappe.pages['agri-leaderboard'].on_page_load = function(wrapper) {
	var page = frappe.ui.make_app_page({
		parent: wrapper,
		title: 'Agri Farm Leaderboard',
		single_column: true
	});
}

frappe.pages["agri-leaderboard"].on_page_load = (wrapper) => {
	frappe.agrileaderboard = new agrileaderboard(wrapper);

	$(wrapper).bind("show", () => {
		// Get which leaderboard to show
		let doctype = frappe.get_route()[1];
		frappe.agrileaderboard.show_leaderboard(doctype);
	});

};
frappe.pages["agri-leaderboard"].refresh = (wrapper) => {
...
class agrileaderboard {
	constructor(parent) {
		frappe.ui.make_app_page({
			parent: parent,
			title: __("Agri Leaderboard"),
			single_column: false,
			card_layout: true,
		});
	...
	get_leaderboard_config() {
		...
		frappe
			.xcall("agri_farm.agri_farm.page.agri_leaderboard.agri_leaderboard.get_leaderboard_config")
			.then((config) => {
	...
	setup_leaderboard_fields() {
		console.log("LEADER BOARD FIELDS")
		var company = ""
		frappe.call({
			method: "agri_farm.agri_farm.page.agri_leaderboard.agri_leaderboard.get_company",
			async: false,
			args: {
				email: frappe.user.name
			},
			callback: function (r) {
				company = frappe.user.name === 'Administrator' ? frappe.defaults.get_default("company") : r.message
            }
		})
...
this.company_select = this.page.add_field({
	...
	render_selected_doctype() {
		this.$sidebar_list.on("click", "li", (e) => {
			...
			frappe.set_route("agri-leaderboard", this.options.selected_doctype);
			this.make_request();
		});
	}
	...
	make_request() {
		frappe.model.with_doctype(this.options.selected_doctype, (data) => {
	...
	get_leaderboard(notify) {
		...
		console.log(this.leaderboard_config[this.options.selected_doctype])
		frappe
			.call("agri_farm.agri_farm.page.agri_leaderboard.agri_leaderboard.get_energy_point_leaderboard", {
